    ...
    <div class="w-full max-w-3xl bg-white shadow-xl rounded-xl p-8 space-y-6">
      ...
      <div class="flex justify-between items-center mb-6">
        <div class="flex-1 text-center">
          <div class={`w-10 h-10 mx-auto rounded-full flex items-center justify-center ${step >= 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-500'}`}>1</div>
          <div class="mt-2 text-sm font-medium text-gray-700">Datos</div>
        </div>
        <div class="flex-1 text-center">
          <div class={`w-10 h-10 mx-auto rounded-full flex items-center justify-center ${step >= 2 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-500'}`}>2</div>
          <div class="mt-2 text-sm font-medium text-gray-700">Entrega</div>
        </div>
        <div class="flex-1 text-center">
          <div class={`w-10 h-10 mx-auto rounded-full flex items-center justify-center ${step >= 3 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-500'}`}>3</div>
      ...
      <div class="space-y-6">
        ...
        {:else}
          <Step3Payment {buyer} {delivery} on:back={back} on:finish={(e) => finishCheckout(e.detail)} />
        {/if}
      </div>
      ...
      {#if processing}
        <div class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div class="bg-white p-6 rounded-lg flex flex-col items-center shadow-lg">
            ...
            <span class="text-blue-600 font-semibold">Procesando tu pedido...</span>
          </div>
        </div>
      {/if}
